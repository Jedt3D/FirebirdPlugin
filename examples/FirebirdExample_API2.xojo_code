...
// ---------------------------------------------------------------------------
// Example 13B: Blob Streaming
// ---------------------------------------------------------------------------

Sub BlobStreamingExample(db As FirebirdDatabase)
  Try
    db.BeginTransaction

    db.ExecuteSQL("CREATE TABLE blob_stream_demo (" + _
      "id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, " + _
      "payload BLOB" + _
      ")")
    ...
    If Not rs.AfterLastRow Then
      ...
      Var preview As MemoryBlock = opened.Read(5)
      Call opened.Seek(0, 0)
      Var wholePayload As MemoryBlock = opened.Read(opened.Length)
      System.DebugLog("Blob length: " + opened.Length.ToString)
      System.DebugLog("Preview bytes: " + preview.Size.ToString)
      System.DebugLog("Whole payload bytes: " + wholePayload.Size.ToString)
      Call opened.Close
    End If
    rs.Close
  ...
  Catch err As RuntimeException
    If db.HasActiveTransaction Then
      Call db.RollbackTransaction
    End If
    System.DebugLog("Blob streaming error: " + err.Message)
  End Try
End Sub
